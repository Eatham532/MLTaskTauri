...
//noinspection ALL
use mltasktauri_lib::{ext_mod, tauri_generate_context};
...
fn main() -> Result<Infallible, Box<dyn Error>> {
    ...
    // 👉 Equivalent to `python -m mltasktauri`,
    // i.e, run the `src-tauri/python/mltasktauri/__main__.py`
    let py_script = PythonScript::Module("mltasktauri".into());
